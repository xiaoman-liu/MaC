...
class ConvBlock(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        super().__init__()
        self.args = args
        self.conv1 = nn.Conv1d(args.input_dim, args.d_model, 3, padding=1)
        self.conv2 = nn.Conv1d(args.d_model, args.d_model, 3, padding=1)
        self.norm = RMSNorm(args.d_model)
...
class ResNetBlock(nn.Module):
    def __init__(self, feature_maps):
        ...
        self.conv1 = nn.Conv1d(in_channels=feature_maps, out_channels=feature_maps, kernel_size=8, padding='same')
        self.bn1 = nn.BatchNorm1d(feature_maps)

        self.conv2 = nn.Conv1d(in_channels=feature_maps, out_channels=feature_maps, kernel_size=5, padding='same')
        self.bn2 = nn.BatchNorm1d(feature_maps)

        self.conv3 = nn.Conv1d(in_channels=feature_maps, out_channels=feature_maps, kernel_size=3, padding='same')
        self.bn3 = nn.BatchNorm1d(feature_maps)

        self.shortcut1 = nn.Conv1d(in_channels=feature_maps, out_channels=feature_maps, kernel_size=1, padding='same')
        self.bn_shortcut1 = nn.BatchNorm1d(feature_maps)

        self.conv4 = nn.Conv1d(in_channels=feature_maps, out_channels=feature_maps * 2, kernel_size=8, padding='same')
        self.bn4 = nn.BatchNorm1d(feature_maps * 2)

        self.conv5 = nn.Conv1d(in_channels=feature_maps, out_channels=feature_maps * 2, kernel_size=5, padding='same')
        self.bn5 = nn.BatchNorm1d(feature_maps * 2)

        self.conv6 = nn.Conv1d(in_channels=feature_maps, out_channels=feature_maps * 2, kernel_size=3, padding='same')
        self.bn6 = nn.BatchNorm1d(feature_maps * 2)

        self.shortcut2 = nn.Conv1d(in_channels=feature_maps, out_channels=feature_maps * 2, kernel_size=1,
                                   padding='same')
        self.bn_shortcut2 = nn.BatchNorm1d(feature_maps * 2)

        self.conv7 = nn.Conv1d(in_channels=feature_maps, out_channels=feature_maps * 2, kernel_size=8, padding='same')
        self.bn7 = nn.BatchNorm1d(feature_maps * 2)

        self.conv8 = nn.Conv1d(in_channels=feature_maps, out_channels=feature_maps * 2, kernel_size=5, padding='same')
        self.bn8 = nn.BatchNorm1d(feature_maps * 2)

        self.conv9 = nn.Conv1d(in_channels=feature_maps, out_channels=feature_maps * 2, kernel_size=3, padding='same')
