...
class ModelArgs:
    ...
    d_state: int = 128
    d_conv: int = 4
    conv_init: Optional[Union[Callable, int]] = None
    expand: int = 2
    headdim: int = 64
...
class Mamba1(nn.Module):
    def __init__(
        ...
        self.process_group = args.process_group
        self.sequence_parallel = args.sequence_parallel
        self.d_inner = self.expand * self.d_model

        self.local_rank = 0 if self.process_group is None else self.process_group.rank()
        self.headdim = args.headdim
        self.d_ssm = self.d_inner if args.d_ssm is None else args.d_ssm // args.world_size
        assert args.ngroups % args.world_size == 0
        self.ngroups = args.ngroups // args.world_size
        assert self.d_ssm % self.headdim == 0
        self.nheads = self.d_ssm // self.headdim
        self.D_has_hdim = args.D_has_hdim
        self.rmsnorm = args.rmsnorm
        self.norm_before_gate = args.norm_before_gate
        ...
        if self.process_group is None:
            ...
            self.in_proj = ColumnParallelLinear(self.d_model, d_in_proj * args.world_size, bias=args.bias,
        ...
        # D "skip" parameter
        self.D = nn.Parameter(torch.ones(self.d_ssm if self.D_has_hdim else self.nheads, device=args.device))
        self.D._no_weight_decay = True

        if self.rmsnorm:
            assert RMSNormGated is not None
            self.norm = RMSNormGated(self.d_ssm, eps=1e-5, norm_before_gate=self.norm_before_gate,
                                     group_size=self.d_ssm // args.ngroups, **factory_kwargs)

        if self.process_group is None:
            self.out_proj = nn.Linear(self.d_inner, self.d_model, bias=args.bias, **factory_kwargs)
        else:
            self.out_proj = RowParallelLinear(self.d_inner * self.world_size, self.d_model, bias=args.bias,
                                              process_group=self.process_group, sequence_parallel=self.sequence_parallel,
                                              **factory_kwargs)

    def forward(self, u, char_embed=None, seqlen=None, seq_idx=None, cu_seqlens=None, inference_params=None):
        """
        ...
        print(f"zxbcdt shape: {zxbcdt.shape}")
...
class CharEmbedding(nn.Module):
    def __init__(self, args: ModelArgs, char_input=None):
        ...
        self.embedding = nn.Embedding(num_embeddings=100, embedding_dim=args.d_model)
        self.mask_value = 0
    ...
    def forward(self, char_input):
        ...
        # Embedding
        char_embedding = self.embedding(char_input)  # Now shape: (batch_size, features, embedding_dim)  # Now shape: (batch_size, features, 1)
