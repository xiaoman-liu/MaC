...
class LinearModel(BaseModel):
    def __init__(self, configs, processed_features, processed_labels):
        ...
        self.logger = logging.getLogger("LinearModel")
        self.select_model = self.configs["select_model"]
        self.mode_config = self.configs[f"{self.select_model}" + "_config"]
    ...
    def infer(self, train_with_all_data=False, result=None):
        ...
        if x_test.isnull().values.any():
            self.logger.warning("There are missing values in the test data, please check!")
            columns_with_nan = x_test.columns[x_test.isnull().any()].tolist()
            self.logger.warning("The columns with missing values are: {}".format(columns_with_nan))
