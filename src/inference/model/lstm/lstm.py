...
from keras.layers import Dense, Conv1D,BatchNormalization,Activation, Input, Flatten, concatenate, TimeDistributed, GlobalAveragePooling1D, Concatenate, Permute, LSTM, Dropout, CuDNNLSTM, Bidirectional
...
class LSTMModel(BaseModel):
    def __init__(self, configs, processed_features, processed_labels):
        ...
        self.mode_config = self.configs["LSTMModel_config"]
    ...
    def infer(self, train_with_all_data=False, result=None):
        ...
        self.char_x_test, self.numer_x_test, self.y_test = map(self.data_reshape,
                                                               (char_x_test, numer_x_test, y_test))
        ...
        if self.isplot:
            ...
            Train_predict_compare(self.configs, y_pred, y_test, save_path)
